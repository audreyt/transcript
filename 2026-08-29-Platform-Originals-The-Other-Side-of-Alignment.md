...
RLVR brings many problems. A model may lose its empathy, lose its humanity, disown its own kin, hack Hugging Face: all side effects of RLVR. But it does one thing at least. It really can make a model extend, by itself, the horizon it thinks across in software engineering. In [METR](https://metr.org/time-horizons)’s terms, the “task time horizon” it can handle—measured by the time a human expert needs for the same task—goes from four hours to eight hours a few months later, to 16 hours a few months after that, and very soon past the distance a person can hold in a single grasp.
